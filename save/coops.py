...
def check_for_coops(coops_dir, monthly_dir, lidar_dir, loc):
    for root, dirs, files in os.walk(os.path.join(coops_dir, loc)):
        for i in sorted(files):
            date = dt.datetime.strptime(i, '%Y-%m.csv')
            if not os.path.isfile(os.path.join(monthly_dir, date.strftime('%Y-%m.txt'))):
                monthly_product(coops_dir, monthly_dir, lidar_dir, loc, date)


def monthly_product(coops_dir, monthly_dir, lidar_dir, loc, date):
    if loc == 'harv':
        coops_data = load_harv_coops(coops_dir, date)
        if coops_data is None:
            return None

        lidar_data = load_lidar(lidar_dir, loc, date)

    elif loc == 'cata':
        coops_data = load_cata_coops(coops_dir, date)
        if coops_data is None:
            return None
    ...
    write_monthly(monthly_dir, data, date)


def write_monthly(monthly_dir, data, month, loc):
    if loc == 'cata':
        with open(os.path.join(monthly_dir, loc, month.strftime('%Y-%m.txt')), 'w') as f:
